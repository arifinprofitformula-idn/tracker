...
Status: **Fase C (Billing + Personal Pro) selesai — backend + UI monetisasi. Fase D (Coach Mode) berikutnya.**
...
| **C** | Billing + Personal Pro | `plans`, `subscriptions`, `billing_transactions`, `webhook_events`, PaymentProvider adapter, entitlement helper server-side, billing/pricing/upgrade/paywall UI | ✅ Selesai |
...
- `PaymentProvider` interface (`src/lib/payment/provider.ts`) + `mockProvider` (`src/lib/payment/mockProvider.ts`) + **`sumopodProvider`** (`src/lib/payment/sumopodProvider.ts`, real gateway — [sumopod.com](https://sumopod.com)), dipilih lewat `getPaymentProvider()` (`src/lib/payment/index.ts`) via env `PAYMENT_PROVIDER` (`mock` default, `sumopod` untuk live). `createCheckoutTransaction`/`processWebhookEvent` menerima provider sebagai parameter yang bisa di-inject, jadi test selalu pakai `mockProvider` eksplisit terlepas dari env aktif — terverifikasi lulus semua meski `PAYMENT_PROVIDER=sumopod` aktif di `.env` lokal.
- Sumopod diverifikasi manual end-to-end lewat sandbox asli (bukan cuma mock): checkout sungguhan menghasilkan `payment_link_url` nyata dari `pay-sandbox.sumopod.com`, row `BillingTransaction` tersimpan dengan `provider: "sumopod"` dan `providerReference` = UUID payment Sumopod. `amountCents` (konvensi Stripe-style, lihat `plans.ts`) dikonversi ke Rupiah aktual (`/100`) saat kirim ke Sumopod. Webhook Sumopod (svix-style: header `svix-id`/`svix-timestamp`/`svix-signature`) pakai `svix-id` sebagai idempotency key (bukan field di body, karena payload Sumopod tidak punya event id top-level) — **belum diverifikasi live** karena butuh `SUMOPOD_WEBHOOK_SECRET` (format `whsec_...`) dari dashboard Settings Sumopod yang belum di-set; tanpa itu semua webhook masuk akan ditolak sebagai invalid signature. **Action item:** isi `SUMOPOD_WEBHOOK_SECRET` di `.env` dan konfigurasi webhook URL (`/api/billing/webhook`) di dashboard Sumopod sebelum menerima pembayaran sungguhan.
- API key Sumopod (sandbox) disimpan di `.env` lokal (git-ignored, **tidak pernah di-commit**) — placeholder-nya ada di `.env.example`.
...
Tambahan monetization UX sebelum Fase D:

- `src/lib/billingSummary.ts` menjadi kontrak server-side untuk UI billing: current workspace, current plan, subscription status, entitlement, usage count, plans/pricing, transaction history, dan scoped payment status.
- `GET /api/billing` sekarang memakai summary contract, dan `GET /api/billing/payment-status?transactionId=...` membaca status transaksi hanya dalam workspace user.
